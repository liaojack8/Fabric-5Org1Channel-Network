...
switch (arg) {
    case 'query' : queryFindData(process.argv[3]); break;
    case 'add' : invokeAddData(process.argv[3], process.argv[4], process.argv[5], process.argv[6]); break;
}

async function queryFindData(vid) { 
    if (vid == undefined) {
        logger.info('Please speficy a name for search.')
        return;
    }
    ...
    let result = await contract.evaluateTransaction('queryData', vid);
    gateway.disconnect();
...
async function invokeAddData(name, subj1, subj2, subj3) {
    logger.info('Begin to add student data %s %s %s %s', name, subj1, subj2, subj3);
    const identityLabel = 'Admin@org1.desl.com';
    const wallet = await initUserWallet(identityLabel);
    const gateway = new Gateway();
    await gateway.connect(path.join(__dirname, './connprofile.json'),
    ...
    try {
        const network = await gateway.getNetwork('channelall');
        const contract = await network.getContract('mycc');
        const transaction = contract.createTransaction('addData');
        ...
        const response = await transaction.submit(name, subj1, subj2, subj3);
        const succ = (eventFired == 1);
        if (succ) {
            logger.info('A new data %s %s %s %s was created. Response: %s', name, subj1, subj2, subj3, response.toString());
...
async function initUserWallet(identityLabel) {
    ...
    const keyPath = path.join(__dirname, "../basic-network/crypto-config/peerOrganizations/org1.desl.com/users/Admin@org1.desl.com/msp/keystore/16d84191b631ad89ea7d79b3f916e5b0363ae1d7da53bad7ce9983062c47be41_sk");
